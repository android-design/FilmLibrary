package com.geekbrains.team.data.movies.similarMovies.repository
...
import com.geekbrains.team.data.movies.similarMovies.service.model.toMovieList
...
class SimilarMoviesRepositoryImpl @Inject constructor(private val api: SimilarMoviesApi) :
    ...
    override fun fetch(id: Int, page: Int?): Single<List<Movie>> {
        return api.getSimilarMovies(
            apiKey = BuildConfig.API_KEY, language = Const.LANGUAGE,
            page = page, id = id
        ).map { response -> response.toMovieList() }
    }
}
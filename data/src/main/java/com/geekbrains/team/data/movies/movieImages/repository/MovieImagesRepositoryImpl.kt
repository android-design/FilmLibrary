...
class MovieImagesRepositoryImpl @Inject constructor(private val api: MovieImagesApi): MovieImagesRepository {
    override fun fetch(id: Int): Single<List<String>> {
        return api.getMovieImages(BuildConfig.API_KEY, Const.LANGUAGE, id).map { response: MovieImagesResponse ->
